...
export class UserPointsRepository {
    ...
    async createUserPoints(userId, initialPoints = 0) {
        const { data, error } = await supabase
            .from("user_points")
            ...
            .single();
        if (error)
            throw new Error(`Erro ao criar pontos: ${error.message}`);
        return data;
    }
    async incrementPoints(userId, amount) {
        const { data, error } = await supabase.rpc("increment_user_points", {
            user_id_input: userId,
            increment_amount: amount,
        });
        if (error)
            throw new Error(`Erro ao incrementar pontos: ${error.message}`);
        return data;
    }
    async decrementPoints(userId, amount) {
        const { data, error } = await supabase.rpc("decrement_user_points", {
            user_id_input: userId,
            decrement_amount: amount,
        });
        if (error)
            throw new Error(`Erro ao decrementar pontos: ${error.message}`);
        return data;
    }
    async ensureUserPoints(userId) {
